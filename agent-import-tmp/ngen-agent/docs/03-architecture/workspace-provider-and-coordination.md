...
- `coding` task 的自治写路径当前已经在全部 current provider mode 上实现为同一套 bounded coding loop；runtime 当前最多允许 3 次 repair cycles，并且在每次真正写文件前最多运行少量 bounded read-only observation commands，在真正写文件前后还可以执行少量 bounded workspace repair commands。repair target 不只可以是 verifier failure，也可以是 workspace-backed criteria gap，其中既包括显式 path / glob criterion，也包括带 readme/docs/config 语义和具体 token 的 criterion。observation command 仍按 read-only executable allowlist 收口，并拒绝 workspace 外路径、repo-root override、明显的 exec-style escape、verifier/build/mutating Go commands、以及 Git `--no-index` / external diff / output file / `rev:path` / deny-path pathspec 绕过。Multica/headless `exec` 的 stdin reader 是薄适配器：它只把 user text blocks 拼接为 prompt，不解析 `metadata`、`system_prompt`、AGENTS.md 或 `.agent_context`，也不从 squad/project UUID 推断 issue id 或合成 issue-execution task。若 user text 本身明确要求运行某个 direct argv command（例如 "Run exactly one `...` invocation."），stdin reader 只生成通用 command-backed criterion；runtime 经普通 repair command policy 执行完全匹配该用户文本的显式命令，并用 completed `command_runs.jsonl` 记录关闭 criteria，不合成命令参数、description 文件、issue comment、delegation 或 fallback artifact。assignment-style prompt（例如包含 assigned issue id、mandatory issue get 和 then complete it）会生成 generic concrete-progress criterion；只读 issue/comment/metadata observation 不能关闭该 criterion，必须看到 durable workspace edit 或非只读 completed repair command evidence。headless prompt 明确要求发布 `reports/`、`progress/`、`validation/` 或 `handoffs/` 下公共 artifact 文件时，会生成 path-backed workspace artifact criteria；docs-lite structural context 不能替代这些目标文件，general/docs-lite task 可针对这些 workspace-backed criteria 进入同一 bounded repair/edit pass。command-backed task 的 verifier gate 不能用 docs-lite structural failure 阻断显式命令执行；命令执行后若 ordinary docs-lite 仍失败，runtime 写入 explicit-command-evidence verification pass，使 done gate 继续由 completed command record、review 与 handoff 收口。外部 CLI mutation（包括 Multica mutation）只是普通 command-backed repair command consumer：`standard` mode 需要 approval，`yolo` mode 可自动执行完全匹配显式 user request 的 direct argv；对 assignment-style concrete-progress criteria，支持 workspace edit planning 的 provider 可以按 workspace guidance / task text 产出 bounded direct argv 外部动作，但 runtime 不维护 Multica subcommand 完成白名单，仍必须记录 command policy 与 replay-safety，失败或 unsafe side-effect attempt 不自动重放。provider 生成的未知外部命令在 standard mode 继续被拒绝。Responses workspace edit/observation/mission-validation 返回 completed 但空文本时，只在 provider adapter 层执行一次通用 retry（提高 max output tokens 并去掉 reasoning），不生成 Multica-specific fallback。workspace repair command 则是更宽的 argv-based action plane，用于 formatter、generator、dependency sync、package install、migration、外部 workspace workflow 动作或 shell-backed repair，并以 budget、timeout、cwd=`workspace root`、durable artifacts 与 verifier/review gate 收口。workspace edit 仍以 patch 为优先路径；`builtin` 通过本地 deterministic repair engine 生成 observation/edit/command plan，`command` 通过 stdin JSON contract 生成 plan，远端 adapter 通过模型生成 plan。durable truth 仍写在 `command_runs.jsonl`、`workspace_edits.jsonl`、events 与最终 verifier/review artifacts
- workspace-backed criteria gap 允许 staged evidence：当 applied workspace edit 或 completed repair command 产生了 durable workspace progress，但目标 artifact criterion 尚未闭合，runtime 会在剩余 repair budget 内继续下一次 criteria repair；没有新 workspace progress 的重复 gap 仍按 stalled 收口。
