...
- invalid JSON、截断、空输出或 schema 校验失败必须把 exact provider error surface 返回到 runtime；Responses 解析失败至少包含 `response_id=...`（若 provider 返回）与 `raw_excerpt=...`，方便复盘截断和格式漂移。Responses structured payload 的空输出或 JSON 语法/截断失败会先重试一次更大、无 reasoning 的 request；重试仍失败时返回包含原始解析失败与重试失败的 provider error，普通 schema/业务校验失败不重试。
