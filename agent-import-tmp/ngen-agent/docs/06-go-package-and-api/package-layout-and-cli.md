...
- `exec` 是 Multica/headless integration surface。它只支持 `--output-format stream-json --input-format stream-json`；stdin 必须是一条 user envelope，可以是 NGEN 顶层形状 `{"protocol":"ngen-stream-json","protocol_version":1,"type":"user","role":"user","content":[...]}`，也可以是 go-cli 兼容的嵌套形状 `{"type":"user","message":{"role":"user","content":[...]}}`。无 `--resume` 时创建 task，有 `--resume` 时按 NGEN task id 继续。`Result.session_id` 等于 task id，final result 同时写顶层 `result` 字段，便于 Multica 直接读取。`--config-scope daemon` 禁止 `<workdir>/ngen.json` 改变 provider/model selection，但仍把 `--workdir` 作为 workspace 和 `.ngen/` artifact root。首次 run 写 `multica/run_metadata.json` 与 `multica/workspace_guidance.json`，resume 时若 metadata 缺失或 config/model fingerprint drift，会 fail closed 为 final `result.status=blocked`。输入读取器是薄适配器：它只接受 `type=user` / `role=user` 的 text content blocks，把非空 text blocks 按 `\n` 拼成 prompt 并交给 task/runtime；它不解析、不复制、也不基于 `metadata`、`system_prompt`、AGENTS.md 或 `.agent_context` 合成 objective、constraints、run role、issue id 或 issue-execution flow，也不能把 squad/project UUID 当作 issue UUID。若用户文本本身明确要求运行某个 direct argv command（例如 "Run exactly one `...` invocation."），adapter 只把 success criteria 表达为通用 command-backed evidence requirement；runtime 随后通过普通 repair command policy 执行完全匹配该用户文本的显式命令，并用 `command_runs.jsonl` 关闭 criteria。assignment prompt 若包含 assigned issue id、mandatory issue get 与 then complete it，会生成 generic concrete-progress evidence criterion；`multica issue get`、`multica issue comment list` 与 `multica issue metadata list` 只算 assignment observation，不能关闭该 criterion。若 prompt 明确要求发布 `reports/`、`progress/`、`validation/` 或 `handoffs/` 下的公共 artifact 文件，adapter 会为每个路径生成 workspace artifact criterion；`.agent_context/issue_context.md` 或 ordinary docs-lite structural pass 不能替代这些目标文件。command-backed task 的 done gate 以 completed command record 为 acceptance truth；即使空 workspace 没有 Markdown 让 docs-lite structural verifier 通过，也不能先用 docs-lite failure 阻止显式命令执行，后续 verifier truth 必须说明 acceptance 由 explicit command evidence 收口。adapter 和 runtime 都不能为显式命令合成参数、description 文件、issue comment、delegation 或 fallback artifact。外部 CLI mutation（Multica command 只是其中一个 consumer 示例）不在 runtime 内建立专用 subcommand 白名单；显式 user-requested direct argv 进入通用 command-backed repair lane，而 assignment concrete-progress task 只能由 durable workspace edit 或 provider/planner 依据 task/workspace guidance 产生的非只读 completed repair command evidence 推进；所有路径都必须留下 `command_runs.jsonl`、stdout/stderr artifact、policy decision 与 replay-safety truth。provider 生成的未知外部命令在 standard mode 仍不自动执行，adapter 也不能直接调用 Multica。
