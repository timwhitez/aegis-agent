...
- `exec` 是 Multica/headless integration surface。它只支持 `--output-format stream-json --input-format stream-json`，stdin 必须是一条 `ngen-stream-json` v1 user envelope；无 `--resume` 时创建 task，有 `--resume` 时按 NGEN task id 继续。`Result.session_id` 等于 task id。`--config-scope daemon` 禁止 `<workdir>/ngen.json` 改变 provider/model selection，但仍把 `--workdir` 作为 workspace 和 `.ngen/` artifact root。首次 run 写 `multica/run_metadata.json` 与 `multica/workspace_guidance.json`，resume 时若 metadata 缺失或 config/model fingerprint drift，会 fail closed 为 final `result.status=blocked`。当 stdin/user/system/context 明确是 Multica issue assignment 且包含 live issue UUID 时，`exec` 创建 command-backed `coding` task，而不是退化为 `general_execution/docs_lite`；该 task 至少要求 `multica issue get <issue-id> --output json` 的 read-only verifier evidence，并将 issue comment、marker、issue runs 与 leader/worker/validator acceptance 转成 command-backed criteria，避免 injected docs 或 `multica-result.md` prose alone 被误当成完成。Leader/master role 可在 live issue 明确要求时通过 issue-scoped `multica squad delegate <issue-id> --role ... --output json` repair command 自动调度 worker/validator；role completion 仍必须由 `multica issue runs <issue-id> --output json` 或等价 command evidence 闭合。Leader 在 worker/validator evidence 尚未完成时发布的公开进度说明使用独立 `multica-progress.md`，最终 marker 与 delegated role marker 仍使用 `multica-result.md`，避免不同阶段的外部评论 mutation 被 replay-safety 误判为同一副作用命令。若远端 workspace edit provider 对已识别 issue marker 返回 empty output text，runtime 可生成显式 fallback event 和最小 result/comment repair plan，但实际 `multica issue comment add` 仍通过 repair command policy 执行；comment-triggered issue runs 必须从 trigger context、live issue run evidence 或 Multica parent-required error evidence 继承 `--parent <trigger-comment-id>`，避免 delegated worker/validator/leader 回复被错误发布为顶层评论。
