...
- `coding`
  ...
  - Multica/headless `exec` 不提供输入读取器级别的 quick-create verifier 例外；stdin adapter 只把 user text blocks 拼成 prompt。若 prompt/runtime 后续产生 Multica observation 或 repair command，verifier/review 只能消费这些命令留下的 `command_runs.jsonl`、stdout/stderr artifact、policy decision、replay-safety 与 read-only issue observation evidence。
