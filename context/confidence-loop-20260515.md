...
- STEP 03 - Actual `AGENTS.md` read and applied at the time: mandatory specs first, then-current pre-Web-first product boundary, no fixed workflow engine, and code fixes must be committed after validation. This historical step has since been superseded by the Web-first AGENTS/spec direction.
