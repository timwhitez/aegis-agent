...
- If the change touches the Web-first console, resume retry recovery, or queue/background notifications, follow the full matrix with `validation/run_webconsole_followup_validation.sh`; it validates the current `go-cli-agent web` operator surface, including embedded-asset checks, a real headless browser interaction smoke, durable retry-policy proof, queue notification dedup evidence, session sidebar filtering/reveal, queue quick-filter chips, and timeline event filtering. The old `validation/run_experimental_webconsole_followup_validation.sh` path remains as a compatibility wrapper.
