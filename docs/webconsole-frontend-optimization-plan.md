...
- `validation/scripts/webconsole_utils_test.mjs` should assert that earlier-message page request sequencing, loading flag, and scroll-preservation height are not stored on the main `state`, and that stale message page responses, disabled loading controls, and prepend scroll restoration remain correct after session switches.
