...
- `app.js` still owns a large global `state` object for selected durable app/session projections, but render-only chat diff cache has been moved into `renderState.chatCache`, transient WebSocket / polling / queued-refresh / layout observer handles live in `runtimeHandles`, page visibility tracking lives in `pageLifecycleViewState`, current navigation view selection lives in `navigationViewState`, WebSocket live relay connection status lives in `connectionViewState`, new-session launch pending state lives in `launchViewState`, current selected-session generating/pending-run affordance state lives in `runViewState`, session detail refresh in-flight/queued flags live in `sessionViewState`, selected queue job inspector selection/detail lives in `queueJobViewState`, inspector tab selection lives in `inspectorViewState`, Settings config request sequencing lives in `settingsViewState`, Workspace navigation/read request sequencing, current directory path, current directory payload, selected tree path, and file preview cache live in `workspaceViewState`, Skills catalog request sequencing, upload pending guard, and current catalog payload live in `skillsViewState`, Overview request sequencing, in-flight/queued refresh flags, and initial load error copy live in `overviewViewState`, History page request sequencing plus in-flight/queued refresh flags live in `historyViewState`, History parent-row expansion ids live in `historyExpansionViewState`, earlier-message paging request sequencing, loading flag, one-render scroll preservation height, and message-window paging facts live in `messagePagingViewState`, transient toast id allocation lives in `toastViewState`, stop-action pending session ids live in `stopActionViewState`, floating tracker panel expansion preferences live in `floatingPanelViewState`, pending Plan Mode input selections live in `planInputViewState`, shortcut help overlay visibility lives in `helpViewState`, composer input empty-state tracking lives in `composerViewState`, next-send interrupt / Goal / Plan Mode composer control intent lives in `composerControlViewState`, transient live activity card copy lives in `activityViewState`, the WebSocket live event relay buffer lives in `liveEventsViewState`, and pending optimistic message display lives in `optimisticMessagesViewState`.
...
- Workspace file tree rendering uses delegated click and keyboard events, stores the selected path in `workspaceViewState.selectedTreePath`, and exposes tree/treeitem semantics for the current navigational file browser.
...
- Current navigation view selection no longer lives on the main global `state`; it is isolated in `navigationViewState` while preserving navigation activation, title updates, localStorage restore/persist, history polling, and skills/workspace/settings lazy refresh.
- Session detail refresh in-flight and queued refresh flags no longer live on the main global `state`; they are isolated in `sessionViewState` while preserving stale detail and stale queue-detail suppression.
- Selected queue job inspector selection/detail no longer lives on the main global `state`; it is isolated in `queueJobViewState` while preserving selected job rendering, stale async detail suppression, and active-descendant polling.
- Inspector tab selection no longer lives on the main global `state`; it is isolated in `inspectorViewState` while preserving tab switching, selected-job focus to Background, invalid-tab fallback to Tasks, and current session rendering.
...
- Workspace file tree selected path and paged file preview cache no longer live on the main global `state`; they are isolated in `workspaceViewState` while preserving stale click suppression, file highlight rendering, paged preview continuation, and load-more error rendering.
- Workspace current directory path and current directory payload no longer live on the main global `state`; they are isolated in `workspaceViewState` while preserving stale directory suppression, breadcrumb/root-chip copy, empty-directory copy, and selected workdir derivation.
- Skills catalog request sequencing and current catalog payload no longer live on the main global `state`; they are isolated in `skillsViewState` while preserving stale catalog response suppression and rendering.
...
- Overview in-flight / queued refresh flags and initial overview load error copy no longer live on the main global `state`; they are isolated in `overviewViewState` while preserving queued refresh coalescing, stale overview suppression, and rail error rendering when no previous overview snapshot exists.
...
- History in-flight and queued refresh flags no longer live on the main global `state`; they are isolated in `historyViewState` while preserving queued page coalescing and stale history suppression.
- Earlier-message paging request sequencing, loading flag, and one-render scroll preservation height no longer live on the main global `state`; they are isolated in `messagePagingViewState` while preserving stale page suppression, disabled loading controls, and prepend scroll restoration after session switches.
- Message-window paging facts (`hasMoreMessages`, `oldestMessageId`, `loadedAllEarlierMessages`, and `messageGapAnchorId`) no longer live on the main global `state`; they are isolated in `messagePagingViewState` while preserving load-earlier availability, stale page suppression, gap filling, and session-switch resets.
...
- Live activity card copy no longer lives on the main global `state`; it is isolated in `activityViewState` while preserving event promotion, restore/load/error text, pending-stage rendering, and existing stale async completion protections.
- Live event relay buffering no longer lives on the main global `state`; it is isolated in `liveEventsViewState` while preserving the capped WebSocket event window and the polling-vs-live-activity overwrite guard.
- Pending optimistic message display no longer lives on the main global `state`; it is isolated in `optimisticMessagesViewState` while preserving pending message rendering and reconciliation against durable messages / steer requests.
...
The large global `state` object still carries selected durable app/session projections, but render-state isolation has removed the known browser-local request guards, display caches, transport handles, and message-window paging projections identified in this plan. The slices have moved the chat stream diff cache out of `state` into `renderState.chatCache`, transient WebSocket / polling / refresh / observer handles into `runtimeHandles`, page visibility tracking into `pageLifecycleViewState`, current navigation view selection into `navigationViewState`, WebSocket live relay connection status into `connectionViewState`, new-session launch pending state into `launchViewState`, current selected-session generating/pending-run affordance state into `runViewState`, session detail refresh in-flight/queued flags into `sessionViewState`, selected queue job inspector selection/detail into `queueJobViewState`, inspector tab selection into `inspectorViewState`, Settings config request sequencing into `settingsViewState`, Workspace request sequencing, current directory path, current directory payload, selected tree path, and file preview cache into `workspaceViewState`, Skills catalog request sequencing, upload pending guard, and current catalog payload into `skillsViewState`, Overview request sequencing, in-flight/queued refresh flags, and rail error copy into `overviewViewState`, History page request sequencing, in-flight/queued refresh flags, current `/api/history` payload, current page, and page size into `historyViewState`, History parent-row expansion ids into `historyExpansionViewState`, earlier-message paging request sequencing, in-flight/render scroll state, and message-window paging facts into `messagePagingViewState`, toast id allocation into `toastViewState`, stop-action pending ids into `stopActionViewState`, floating tracker panel expansion preferences into `floatingPanelViewState`, pending Plan Mode input selections into `planInputViewState`, shortcut help overlay visibility into `helpViewState`, composer input empty-state tracking into `composerViewState`, next-send interrupt / Goal / Plan Mode composer control intent into `composerControlViewState`, live activity card copy into `activityViewState`, the WebSocket live event relay buffer into `liveEventsViewState`, and pending optimistic message display into `optimisticMessagesViewState`.
...
- `validation/scripts/webconsole_utils_test.mjs` should assert that WebSocket live relay connection status is not stored on the main `state`, and that connection copy, offline styling, fallback polling, and interval selection still work.
- `validation/scripts/webconsole_utils_test.mjs` should assert that new-session launch pending state is not stored on the main `state`, and that duplicate-start suppression, pending controls, durable-session adoption cleanup, and stale launch completion protections still work.
- `validation/scripts/webconsole_utils_test.mjs` should assert that current selected-session generating/pending-run affordance state is not stored on the main `state`, and that steer/interrupt controls, pending-stage rendering, input labels, and active polling still work.
- `validation/scripts/webconsole_utils_test.mjs` should assert that session detail refresh in-flight/queued flags are not stored on the main `state`, and that stale same-session detail and queue-detail responses remain ignored.
- `validation/scripts/webconsole_utils_test.mjs` should assert that selected queue job inspector selection/detail is not stored on the main `state`, and that selected job rendering, stale detail suppression, and active-descendant polling still work.
- `validation/scripts/webconsole_utils_test.mjs` should assert that inspector tab selection is not stored on the main `state`, and that timeline tab rendering plus invalid-tab fallback still work.
...
- `validation/scripts/webconsole_utils_test.mjs` should assert that Workspace selected path and file preview cache are not stored on the main `state`, and that stale file-click suppression plus paged preview continuation still work.
- `validation/scripts/webconsole_utils_test.mjs` should assert that Skills catalog request sequencing and current catalog payload are not stored on the main `state`, and that stale Skills catalog responses remain ignored.
...
- `validation/scripts/webconsole_utils_test.mjs` should assert that Overview request sequencing, in-flight/queued refresh flags, and initial overview load error copy are not stored on the main `state`, and that stale queued Overview responses plus rail error rendering remain correct.
- `validation/scripts/webconsole_utils_test.mjs` should assert that History page request sequencing, in-flight/queued refresh flags, current page, page size, and current `/api/history` display payload are not stored on the main `state`, and that stale queued History page responses plus UI-state page restore remain correct.
- `validation/scripts/webconsole_utils_test.mjs` should assert that earlier-message page request sequencing, loading flag, scroll-preservation height, and message-window paging facts (`hasMoreMessages`, `oldestMessageId`, `loadedAllEarlierMessages`, `messageGapAnchorId`) are not stored on the main `state`, and that stale message page responses, disabled loading controls, load-earlier availability, gap filling, and prepend scroll restoration remain correct after session switches.
...
- `validation/scripts/webconsole_utils_test.mjs` should assert that live activity card copy is not stored on the main `state`, and that pending-stage rendering still reads the current activity.
- `validation/scripts/webconsole_utils_test.mjs` should assert that the live event relay buffer is not stored on the main `state`, and that capped buffering plus reset behavior still work.
- `validation/scripts/webconsole_utils_test.mjs` should assert that pending optimistic message display is not stored on the main `state`, and that pending rendering plus reset behavior still work.
