...
func TestProbeProviderCommandRejectsFinishPayloadOutsideClosedSchema(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{
	...
	cfg.Providers["probe-extra-finish"] = config.Provider{
		APIProvider:       "openai-compatible",
		APIKeyEnv:         "PROBE_EXTRA_FINISH_CLI_API_KEY",
		BaseURL:           server.URL,
		Model:             "probe-model",
		RequestTimeoutSec: 3,
	}
	t.Setenv("PROBE_EXTRA_FINISH_CLI_API_KEY", "test-key")
	restore := runnerLoader
	runnerLoader = func(string, string) (coreRunner, *config.Config, error) {
		return runtime.NewCoreRunner(cfg), cfg, nil
	}
	...
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := Run(context.Background(), []string{"probe-provider", "--json", "--provider", "probe-extra-finish"}, &stdout, &stderr)
	var exitErr ExitError
	if !errors.As(err, &exitErr) || exitErr.Code != 1 {
		t.Fatalf("expected exit code 1 for extra finish field, err=%v stdout=%s stderr=%s", err, stdout.String(), stderr.String())
	}
	var payload probeProviderJSON
	if err := json.Unmarshal(stdout.Bytes(), &payload); err != nil {
		t.Fatalf("unmarshal probe JSON: %v output=%s", err, stdout.String())
	}
	if payload.ErrorClass != "response_parse_error" || !strings.Contains(payload.Error, `unexpected field "unexpected"`) || payload.FinishMessage != "" {
		t.Fatalf("closed-schema finish failure was not exposed by CLI: %#v", payload)
	}
}
