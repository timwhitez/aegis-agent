...
func (c *CompletionController) parentCoordinationGate(toolName string) (string, string, error) {
	...
	return "parent_coordination", fmt.Sprintf("Parent-coordination gate: unresolved child or queue work remains before finish (children: %s; jobs: %s). Prompt running or resumable child work to converge with agent_prompt, wait for completion with agent_wait, stop queued or budget-paused blocked work that is no longer needed with agent_stop, or explicitly resolve the outstanding work before finishing.", joinPromptItems(coordination.UnresolvedChildSessions), joinPromptItems(coordination.UnresolvedQueueJobs)), nil
}
