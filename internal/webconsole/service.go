...
func (s *Service) handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	// Use a runner to subscribe to events
	runner := runtime.NewRunner(s.cfg)
	...
	go func() {
		for {
			select {
			case evt := <-sub:
				// Translate internal events to frontend format
				msg := map[string]any{
					"type": "engine_event",
				...
				// Special handling for messages to match chat format
				if evt.Type == "message.created" || evt.Type == "message.updated" {
					if data, ok := evt.Data["message"].(map[string]any); ok {
						msg["type"] = "message"
	...
	for {
		...
		if data.Type == "chat" {
			...
			// Note: This is a simplified version. For a full implementation, we'd use s.startSession correctly.
			go func() {
				req := runtime.StartRequest{
					Prompt: data.Message,
				}
				_, _ = s.startSession(req)
			}()
		} else if data.Type == "stop" {
			s.handleInterruptSession(nil, data.SessionID)
		}
	}
	close(stop)
}
