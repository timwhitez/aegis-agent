...
- Multica 侧改动集中在第 6 章：只新增 `ngen` backend、factory/model/version/thinking 注册、daemon env/config、execenv brief/skills path 和测试；不要求修改 NGEN 内部 artifact schema 以外的 Multica 调度事实源。
- 共享 wire contract 只在第 4 章和 5.4/6.1 定义：stdout NDJSON、`Result.SessionID=task_id`、model route、blocked status、usage、handoff refs。共享层不能扩展成第二套 workflow engine。
...
type ModelThinkingInfo struct {
    SupportedLevels []ThinkingLevelInfo `json:"supported_levels,omitempty"`
...
- 新增 `internal/multica/run_metadata.go`，定义并持久化 Multica/NGEN run route artifact。
- 必要时在 `ngenrt.Service` 增加小的 facade 方法，避免 CLI 层直接读太多 artifact internals。
- 不复用当前 `runStreaming` 作为协议实现：当前 `runStreaming` 是 `svc.Run/Resume/Auto` 返回后批量打印事件，不是真实时流。`ngen exec` 必须新增同进程 artifact tailer 或 runtime event sink，在 run 过程中 emit status/log/tool projection，并在 service 返回后 flush 未见事件再输出 final result。
- live stream 实现约束：stdout 必须由单一 encoder goroutine 串行写；tailer/sink 维护 `event_id` high-water mark；service 返回后按 high-water flush 未发事件，避免重复和乱序。
...
ngen exec \
  ...
  [--config <file>] \
  [--resume <task-id>] \
  [--model <provider-mode/model>] \
  [--role orchestrator|worker|validator|reviewer] \
  [--timeout-seconds <n>] \
  [--permission-mode standard|yolo]
```
...
- `--model` route 必须写入 durable route artifact，并在 final result `metadata.model_route/provider_mode/provider_model` 和 handoff digest 中回显。首次创建 task 时记录 selected route；resume 时从 durable route artifact 读取原 route，若 Multica 传入不同 route，MVP 应 fail closed 并返回清晰 blocked/failed result，除非用户显式允许 model switch。
...
- `PermissionModeID`: 只来自 daemon-owned config/fixed default/明确 backend-owned option；CustomArgs、ExtraArgs、`MULTICA_NGEN_ARGS` 都不能覆盖。默认 `standard`。

durable route artifact:
...
- Resume: read the artifact before provider execution; if absent for a legacy task, write it only when `--model` is empty or when an explicit `--allow-route-adopt` option exists. Otherwise fail closed with `result.status="blocked"` and `status_reason_code="multica_route_missing"`.
- Route mismatch: do not mutate the artifact; emit final `result.status="blocked"` or `failed` with `status_reason_code="multica_route_mismatch"`, preserve `SessionID=task_id`, and include old/new route in bounded metadata.
...
- `StateBlocked` or approval/input wait -> NGEN result status `blocked`。MVP 中 Multica backend 可以返回 `Result.Status="blocked"`，让 daemon 的 existing default branch 进入 blocked/fail-close 路径；如果产品层暂不支持 first-class needs-input，也必须把 `Error`/`Output` 标成 `agent_blocked` 并保留 handoff、worker parent action 和 `SessionID=task_id`，不能泛化成普通 agent error。
...
- NGEN should read this as normal workspace guidance, not as provider-specific replay state. If MVP does not implement NGEN-side AGENTS.md ingestion, Multica may still inject the file for consistency, but acceptance must state that NGEN is not yet consuming it.
...
- `ngen models --json` returns route IDs and uses same config as exec.
- `openai-response/gpt-5.5` splits into provider mode + model and round-trips.
- `exec` without resume creates a task and emits `system`, live/bounded `status` or `log` during execution, and final `result`.
...
- first run writes `.ngen/tasks/<task_id>/multica/run_metadata.json` with `model_route/provider_mode/provider_model/config_fingerprint`; resume with same route succeeds, resume with different route fails closed or requires explicit allow-switch flag.
- legacy task without route metadata returns `status_reason_code=multica_route_missing` unless `--model` is empty or explicit `--allow-route-adopt` exists.
- stdout NDJSON remains valid with stderr diagnostics present.
- final result is the last stdout line.
- blocked custom/user args cannot override protocol-critical flags or `--permission-mode`.
...
- `opts.Model` route -> `--model <route>`.
- `opts.ThinkingLevel` -> only pass if NGEN defines a native thinking flag; otherwise ignore with a debug log, not failure.
...
var ngenBlockedArgs = map[string]blockedArgMode{
    ...
    "--allow-route-adopt": blockedStandalone,
...
- `Result.Status="blocked"` is allowed for NGEN blocked/approval/input wait. Current Multica daemon may route unknown/non-completed statuses through a blocked/fail-close path; parser tests must prove this status is never treated as `completed`.
...
- `ModelSelectionSupported("ngen")` returns true if NGEN accepts route model IDs. If only `builtin/default` is available, still true but UI should show the runtime catalog.
...
- If NGEN exposes fixed reasoning levels, add provider enum.
- If NGEN only passes model/provider defaults, keep NGEN absent and accept only empty `ThinkingLevel`. Do not copy Codex/Claude enum unless NGEN actually supports it.
...
```go
if e, ok := probe("MULTICA_NGEN_PATH", "ngen", "MULTICA_NGEN_MODEL"); ok {
    agents["ngen"] = e
}
```
...
Recommendation: do not use `MULTICA_NGEN_ARGS` for config-critical values like `--config`, `--workdir`, `--model`, `--provider-mode`, `--permission-mode`, or protocol format. Prefer daemon env/config so model discovery and execution stay aligned.
...
- custom args and daemon-wide `ExtraArgs` cannot override `--output-format`, `--input-format`, `--workdir`, `--resume`, `--model`, `--config`, `--timeout-seconds`, or `--permission-mode`.
- custom args and daemon-wide `ExtraArgs` cannot override `--config-scope`, `--ignore-workdir-provider-config`, or `--allow-route-adopt`.
...
- fake `ngen models --json` returns route IDs and labels.
...
- `ModelSelectionSupported("ngen")` matches chosen behavior.
- `DetectVersion` parses `ngen --version`; `CheckMinVersion("ngen", ...)` works once min version is set.

Daemon/execenv tests:

- `MULTICA_NGEN_PATH` / `MULTICA_NGEN_MODEL` probe populates agents map.
...
- fake end-to-end backend script receives expected args/stdin and emits final `result`.
...
- model ID 必须是 route key，不只是 provider-native model 名。
- CustomArgs/ExtraArgs 不能覆盖协议关键 flag、config/workdir/resume/model route 或 permission mode。
...
- `openai-response/gpt-5.5` 等 route ID 可逆拆分。
- Multica `agent.model` 传入的字符串就是 NGEN route ID，不在 Multica 中改写成 provider-native ID。
- first run、resume、final handoff 中的 `model_route/provider_mode/provider_model` 都来自 durable `multica/run_metadata.json`，且保持一致；resume route mismatch fail closed，除非显式允许切换。
...
- 用户 custom env/args 不能改变 protocol format、workdir、resume id、model route、permission mode、config ownership。
...
3. 添加 `models --json`，输出 route model catalog。
4. 添加 `internal/multica` stream structs、usage parser、encoder。
5. 添加 `ngen exec` headless command：stdin envelope -> create/resume task -> live event tailer/sink -> `Run/Resume/Auto` -> flush -> final result。
...
6. 添加 fake-ngen backend E2E，覆盖 completed、blocked、resume。

Phase C：multi-agent hardening

1. 扩展 NGEN handoff projection，覆盖 worker settlement/reconcile/trust 字段。
2. 在 Multica UI/API 中把 NGEN `blocked` / `requires-parent-action` 展示为可恢复 parent-action 状态，而不是普通失败；MVP 已保留 status/handoff/session id。
3. 对 NGEN worker handoff 增加 parent-action display，但仍不让 Multica 解析 `.ngen/` 为权威。
4. 视需要新增 Multica runtime profile：`ngen-large-project`，只改变 prompt/skills/profile，不改变 protocol。
...
5. blocked smoke 证明 `result.status=blocked`、`SessionID=task_id`、worker `requires_parent_action` 都被保留，下一次 resume 仍使用同一 task id。
6. worker smoke 证明 child result trust fields 出现在 final handoff。
...
```text
MULTICA_NGEN_PATH=/path/to/ngen MULTICA_NGEN_MODEL=openai-response/gpt-5.5 multica daemon
```

Expected:

- model list endpoint shows NGEN route IDs。
- first run returns `Result.SessionID=<ngen task_id>`。
- resume uses the same task id and NGEN continues from `.ngen/` artifacts。
- blocked run preserves `Result.SessionID=<ngen task_id>` and a structured handoff; next resume uses the same task id。
- final output contains concise handoff summary plus artifact refs。

## 11. Open decisions

- NGEN 是否把 headless command 命名为 `exec` 还是 `multica run`。推荐 `exec`，因为 Multica backend command 更短，且语义接近其他 subprocess runtimes。
- NGEN 是否支持 `--thinking-level`。只有 NGEN 自身能把该值稳定映射到 provider mode 时才添加；否则 Multica 对 NGEN 只接受空值。
- NGEN blocked 状态在 Multica UI 中是否新增 first-class `blocked/needs_input` 仍是产品层开放项；协议/backend MVP 已要求保留 `blocked`、handoff、parent action 和 `SessionID=task_id`，不能退化为普通 agent error。
- `NGEN_CONFIG` MVP 已决策为 daemon-owned。若未来要支持 per-agent custom config，必须先扩展 model discovery env/config/workdir 通道和 cache key。
...
- Multica 能创建 NGEN task、接收实时或 bounded-near-real-time status/log/text、拿到 final result；如果 NGEN 只能 batch 输出，不能宣称满足本 MVP。
- `Result.SessionID` 可用于下一次 resume，且恢复同一个 NGEN `task_id`。
- blocked/approval/input wait 不丢 session id、handoff 或 parent action，且不会被误标为 completed。
- model route 在 first run、resume 和 final handoff 中一致；route mismatch 有明确 fail-closed 语义。
- stdout/stderr、usage、model route、custom args、skills path、AGENTS.md 注入都通过测试。
