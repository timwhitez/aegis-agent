...
- 默认 probe 只有在 stop reason 为 `tool_use`、恰好返回一个 `finish`，且其 arguments 能通过 runtime `finish` 的同一份 closed-schema + 必填非空 `message` validator 时成功；advertised probe schema 与真实执行 schema 必须同源并拒绝 unknown field、trailing JSON 和类型不匹配；thinking-observation probe 只有在 stop reason 为 `done_candidate` 且不含 tool call 时成功
