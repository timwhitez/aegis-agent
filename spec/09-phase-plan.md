...
- `node --check internal/webconsole/assets/*.js internal/webconsole/assets-v2/*.js` 覆盖两套内嵌前端语法，`node --test validation/scripts/webconsole_utils_test.mjs` 覆盖前端状态机、异步竞态与模块 API contract
