...
- `mission` 的 features / milestones 可以携带 `task_ids`、`child_session_ids`、`queue_job_ids` 等显式执行关联；默认创建 goal / mission 不强制生成 task graph，也不让 task graph 承担目标完成判定。只有用户启用 `create_tasks_from_plan` 或模型显式调用 task 工具时，才把计划项同步到 durable tasks。
