...
本目录定义 `go-cli-agent` 作为 Multica 后端 agent runtime 的二次开发方案。本轮只写文档，不修改代码。

## Review 结论

截至 2026-06-01，当前 `go-cli-agent` 不能直接接入 Multica。复核依据：

- `go-cli-agent` 当前工作树代码：`internal/app/app.go`、`internal/runtime/engine.go`、`internal/runtime/runner.go`、`internal/config/config.go`
...
| 差距 | 结论 | 修正方案 |
| --- | --- | --- |
| stdout 协议不兼容 | 阻塞 | `go-cli-agent` 新增 `--output-format stream-json`，由 adapter 把 runtime events 转成 Multica 可消费的 NDJSON |
| stdin 协议不兼容 | 阻塞 | `go-cli-agent exec` 新增 `--input-format stream-json`，从 stdin 读取一条 user envelope 作为 prompt |
| 顶层版本发现缺失 | 阻塞 | `go-cli-agent --version` 输出含 semver 的版本行，供 Multica `DetectVersion` 使用 |
| 模型发现缺失 | 阻塞 | `go-cli-agent models --json` 输出 Multica `agent.Model` 兼容结构 |
...
| model ID 只写 provider 原生 model | Multica 只有 `opts.Model`，无法同时表达 go-cli-agent provider profile | `models --json` 使用 `<provider>/<model>` route ID，Multica backend 拆成 `--provider` + `--model` |
...
| 侧 | 独立交付物 | 可独立验证 |
| --- | --- | --- |
| `go-cli-agent` | `internal/streamjson`、CLI flags、`models --json`、`--version`、event 字段补齐 | 用 shell 管道和 `jq` 校验 NDJSON、usage、tool call id、models |
| Multica | `server/pkg/agent/gocli.go`、factory/model/version/thinking 注册、daemon probe、execenv 路径 | 用 mock `go-cli-agent` 脚本驱动 `go test ./server/pkg/agent` 和 daemon config/execenv 单测 |
...
- Multica 可以承担 mission orchestrator / Mission Control 职责；`go-cli-agent` 仍只承担本地 agent runtime、session facts、tool execution 和 provider adapter。
- 验证契约必须在编码前定义，并与实现聊天上下文分离；Multica mission store 或 go-cli-agent Goal 内部计划可以保存 contract，`wire-protocol.md` 只传递可选 metadata / artifact ref。
...
- 角色化模型选择由 Multica 策略层决定，通过 `--provider` / `--model` / `--thinking-level` 传给 go-cli-agent；provider replay 差异仍留在 go-cli-agent adapter 层。
- discipline 由系统事实源、coverage check、handoff check 和 plan approval gate 提供；智能仍由模型和 skill/prompt 决策提供，不能把 runtime 写成任务专用 DAG。

## 集成后命令

Multica daemon 启动 `go-cli-agent` 的标准形态：
...
- 不把 Multica 写成 go-cli-agent provider adapter；Multica 只管理子进程和 wire protocol。
- 不改 `go-cli-agent` provider adapters、session store schema 或 Web console 权威状态。
- 不让 `go-cli-agent` 默认 Web 页面围绕 Multica 做产品复杂化；本方案只增加 CLI/adapter 兼容面。
...
- `go-cli-agent --version` 可被 Multica `versionRe` 解析。
- `go-cli-agent models --json` 可直接 unmarshal 到 Multica `[]agent.Model` 等价结构，且每个发现模型的 `id` 是 `<provider>/<model>` route key。
- `go-cli-agent exec --output-format stream-json --input-format stream-json` stdout 每行都是合法 JSON，stderr 不混入协议。
