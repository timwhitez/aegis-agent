...
目标：在 Multica 中新增 `gocli` backend，使 daemon 可以启动、观测、恢复 `aegis-agent`。
...
- Multica execenv 会按 provider 写 runtime context 和 skills；`gocli` 需要按 aegis-agent 的 `AGENTS.md` + `./skills` 约定接入。
...
| `server/pkg/agent/models.go` | 修改 | `ListModels("gocli")` 动态调用 `aegis-agent models --json` |
...
配置路径必须特别处理：Multica 当前 `ListModels(ctx, provider, executablePath)` 只传 executable path，不传 execution args。若 aegis-agent 使用非默认 config，推荐在 daemon 环境设置 `AEGIS_AGENT_CONFIG=/path/to/config.yaml`，使 `models --json` 与 `exec` 读取同一配置；`MULTICA_GOCLI_ARGS --config ...` 只影响 task execution，不能作为模型发现的配置来源。
...
- 不解析 aegis-agent session 目录；Multica 只消费 stdout 协议和 Result.SessionID。
...
- aegis-agent owns：本地 runtime loop、tool execution、provider adapter、session id、events、messages、artifacts。
- `gocli` backend 只解析 [../wire-protocol.md](../wire-protocol.md) 的 stdout/stdin 协议，不读取 `aegis-agent` 私有 session schema。
