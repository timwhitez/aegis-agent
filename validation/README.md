...
- 同一个脚本还会验证 embedded shell / JS / CSS 资产可被本地 service 提供，并用 headless Chrome 跑一条真实浏览器交互链，覆盖 Settings / Workspace / Skills / Sessions / Session 导航、主 prompt start、durable session chrome、tool cards、timeline 可见性和 history clear；API-backed queue job 完成只通过后端 queue detail / 文件事实源验证，浏览器 smoke 不再要求 Background / Queue 前端入口、`Open job` 或 selected job facts 面板；standalone Overview / Worker Pool drilldown、queue notification dedup、retry proof 和 worker API 行为由同一脚本的 API / 文件事实检查或服务层测试覆盖
- `./validation/run_budget_browser_smoke.sh [output-dir]` 不依赖外部 provider 或 API key：它启动本地 scripted Responses provider 与真实 WebConsole，用 headless Chrome 验证 Global Turn Guard 默认 Off/全局 scope、Sub-agent Budget 默认 Off、`30m`/`2h` duration 保存、Explorer role override 保存/重新读取、canonical config/API/audit round-trip，并跑通 foreground child budget pause -> parent extend/resume -> complete 与 background child budget pause -> parent cancel/settle。scripted provider 还会生成 70000-byte command output，验证 complete artifact pointer、`read_file` byte page、`read_session_history` record/content continuation，以及 Context inspector 的 lazy load/Refresh；smoke 同时检查 compact inspector 的 attempt/usage/remaining/deadline/reason/source、terminal notification 不残留 parent actions、cancelled 不进入 failure telemetry。传入 output-dir 时保留 JSON/DOM/durable files；省略时使用临时目录并在结束后清理
