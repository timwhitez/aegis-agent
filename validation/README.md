...
- `run_webconsole_followup_validation.sh` 是当前稳定的 focused live 入口：它用一个本地 fault-injection proxy 稳定触发 `continue` 阶段的 `provider.retry`，会先制造一个真实 failed queue canary 作为 durable evidence，然后在真实 Web-first console queue 完成后强制一次 stale-running reconcile，检查 parent `background.jsonl` 仍按 `queue_job_id` 去重
