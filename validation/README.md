...
- `./validation/run_budget_browser_smoke.sh [output-dir]` 不依赖外部 provider 或 API key：它启动本地 scripted Responses provider 与真实 WebConsole，用 headless Chrome 验证 Global Turn Guard 默认 Off/全局 scope、Sub-agent Budget 默认 Off、`30m`/`2h` duration 保存、canonical config/API/audit round-trip，并跑通 foreground child budget pause -> parent extend/resume -> complete 与 background child budget pause -> parent cancel/settle。它同时检查 compact inspector 的 attempt/usage/remaining/deadline/reason/source、terminal notification 不残留 parent actions、cancelled 不进入 failure telemetry。传入 output-dir 时保留 JSON/DOM/durable files；省略时使用临时目录并在结束后清理
