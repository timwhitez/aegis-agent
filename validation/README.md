...
`experimental web` / retry-resume / queue follow-up 的稳定 live 入口：
...
- `run_experimental_webconsole_followup_validation.sh` 是当前稳定的 focused live 入口：它用一个本地 fault-injection proxy 稳定触发 `continue` 阶段的 `provider.retry`，并在真实 webconsole queue 完成后强制一次 stale-running reconcile，检查 parent `background.jsonl` 仍按 `queue_job_id` 去重
...
- 同一个脚本还会验证 embedded shell / JS / CSS 资产可被本地 service 提供，并用 headless Chrome 跑一条真实浏览器交互链，覆盖 role-aware start、tasks/children/queue tab 切换、continue、worker 更新、queue submit、queue view、queue-links 通知和手动刷新
