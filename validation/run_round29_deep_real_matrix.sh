...
MATRIX_LABEL="${GO_CLI_AGENT_MATRIX_LABEL:-round29-deep-real-matrix}"
...
finalize_scenario "RT04" "Forced Compaction Proof Drill" "$RT04_EXIT" "$RT04_RAW" "${ARTIFACT_DIR}/rt04-forced-compaction-proof.md" "$(extract_session_id "$RT04_RAW")"
...
Diagnose the incident in this workspace, build a durable task graph with at least four tasks and one explicit dependency edge, and keep a durable project-memory stack under reports/spec.md, reports/plan.md, reports/progress.md, and reports/validation.md.
Use the logs and service_config.json to build a confirmed timeline before you propose a root cause.
Write reports/incident-summary.md and reports/taskboard-summary.md.
Then call finish with a one-line summary."
RT05_RAW="${RAW_DIR}/rt05-incident-task-graph.jsonl"
run_exec "$RT05_PROMPT" "$RT05_RAW" "$INCIDENT_DIR" 300
RT05_EXIT=$?
RT05_SESSION_ID="$(extract_session_id "$RT05_RAW")"
if [[ -n "$RT05_SESSION_ID" ]]; then
	"${AGENT_BIN}" tasks "$RT05_SESSION_ID" --config "$CONFIG_PATH" --json >"${RAW_DIR}/rt05-incident-taskboard.json" 2>&1
	RT05_TASKS_EXIT=$?
	RT05_EXIT="$(merge_exit_code "$RT05_EXIT" "$RT05_TASKS_EXIT")"
else
...
finalize_scenario "RT05" "Incident Triage And Durable Task Graph" "$RT05_EXIT" "$RT05_RAW" "${ARTIFACT_DIR}/rt05-taskboard-summary.md" "$RT05_SESSION_ID"
...
This is a real multi-package repair, not a toy patch. Keep durable memory under reports/spec.md, reports/plan.md, reports/progress.md, and reports/validation.md if the work spans multiple steps.
Run the narrowest failing tests first, then fix all failing go tests across internal/api, internal/config, and internal/quota.
Preserve the public contract that internal_id stays private.
Write reports/change-summary.md with sections: root cause, files changed, verification, remaining risks.
Then call finish with a one-line summary."
RT11_RAW="${RAW_DIR}/rt11-platform-go-fix.jsonl"
run_exec "$RT11_PROMPT" "$RT11_RAW" "$PLATFORM_GO_DIR" 420
RT11_EXIT=$?
copy_if_present "${PLATFORM_GO_DIR}/reports/change-summary.md" "${ARTIFACT_DIR}/rt11-platform-go-change-summary.md"
finalize_scenario "RT11" "Platform Go Multi-Package Repair" "$RT11_EXIT" "$RT11_RAW" "${ARTIFACT_DIR}/rt11-platform-go-change-summary.md" "$(extract_session_id "$RT11_RAW")"
...
This is a real multi-module repair. Keep durable memory under reports/spec.md, reports/plan.md, reports/progress.md, and reports/validation.md if the work spans multiple steps.
Run the narrowest failing tests first, then fix all failing pytest cases across app/config.py, app/rules.py, and app/report.py.
Write reports/change-summary.md with sections: root cause, files changed, verification, remaining risks.
Then call finish with a one-line summary."
RT13_RAW="${RAW_DIR}/rt13-platform-py-fix.jsonl"
run_exec "$RT13_PROMPT" "$RT13_RAW" "$PLATFORM_PY_DIR" 420
RT13_EXIT=$?
copy_if_present "${PLATFORM_PY_DIR}/reports/change-summary.md" "${ARTIFACT_DIR}/rt13-platform-py-change-summary.md"
finalize_scenario "RT13" "Platform Python Multi-Module Repair" "$RT13_EXIT" "$RT13_RAW" "${ARTIFACT_DIR}/rt13-platform-py-change-summary.md" "$(extract_session_id "$RT13_RAW")"
...
Inspect only these current-run artifacts: ${WORKSPACE_ARTIFACT_DIR}/rt05-incident-summary.md, ${WORKSPACE_ARTIFACT_DIR}/rt05-taskboard-summary.md, ${WORKSPACE_ARTIFACT_DIR}/rt06-docset-continue-brief.md, ${WORKSPACE_ARTIFACT_DIR}/rt08-delegate-review.md, ${WORKSPACE_ARTIFACT_DIR}/rt09-background-summary.md, ${WORKSPACE_ARTIFACT_DIR}/rt11-platform-go-change-summary.md, ${WORKSPACE_ARTIFACT_DIR}/rt13-platform-py-change-summary.md.
Also inspect only these notes and source references: ${WORKSPACE_RUN_DIR}/notes/scenario-index.tsv, ${WORKSPACE_RUN_DIR}/notes/rt07-steer-metadata.txt, ${WORKSPACE_RUN_DIR}/notes/rt08-delegate-metadata.txt, ${WORKSPACE_RUN_DIR}/notes/rt09-queue-metadata.txt, go-cli-agent/spec/01-runtime-architecture.md, go-cli-agent/spec/12-task-system.md, go-cli-agent/internal/runtime/prompt.go, go-cli-agent/internal/runtime/project_memory.go, go-cli-agent/internal/session/store.go, and go-cli-agent/internal/tools/registry.go.
Use this live evidence to judge whether go-cli-agent now demonstrates real durable project-memory and task-system behavior in actual runs rather than only in spec text.
Write ${ABS_ARTIFACT_DIR}/rt15-task-memory-traceability.md with sections: confirmed live behavior, findings, unresolved questions, next validation moves.
Then call finish with a one-line summary."
