...
MATRIX_LABEL="${GO_CLI_AGENT_MATRIX_LABEL:-round10-complex-matrix}"
...
Write ${ABS_ARTIFACT_DIR}/rt01-architecture-audit.md with sections: architecture map, findings, unresolved questions, smallest next fixes.
...
Inspect only spec/03-provider-contracts.md, internal/provider/openai.go, internal/provider/anthropic.go, internal/provider/google.go, internal/provider/types.go, internal/runtime/runner.go, and internal/runtime/engine.go.
Ignore validation/runs, validation/sessions, reports, bin, tmp, and generated artifacts.
Prefer targeted retrieval with small read_file slices. Keep validated alignments separate from confirmed drifts, and label findings with severity and confidence.
...
record_scenario "RT03" "Top-Level Markdown Corpus Synthesis" "$RT03_RAW" "${ARTIFACT_DIR}/rt03-top-level-md-synthesis.md" "$(extract_session_id "$RT03_RAW")"
...
Inspect only README.md, AGENTS.md, spec/11-spec-audit-and-traceability.md, internal/runtime/prompt.go, internal/runtime/review_guard.go, internal/review/report.go, validation/skills/repo_audit/SKILL.md, and validation/skills/review_pipeline/SKILL.md.
Ignore validation/runs, validation/sessions, reports, bin, tmp, and generated artifacts.
Audit whether go-cli-agent already has a serious review pipeline for findings, evidence, confidence, and unresolved questions.
Write ${ABS_ARTIFACT_DIR}/rt04-review-pipeline.md with sections: current pipeline, findings, missing pieces, smallest next fixes.
Then call finish with a one-line summary."
RT04_RAW="${RAW_DIR}/rt04-review-pipeline.jsonl"
run_exec "$RT04_PROMPT" "$RT04_RAW" "$ROOT_DIR" 300
record_scenario "RT04" "Review Evidence Pipeline Drill" "$RT04_RAW" "${ARTIFACT_DIR}/rt04-review-pipeline.md" "$(extract_session_id "$RT04_RAW")"
...
Also inspect go-cli-agent/internal/runtime/project_memory.go.
Focus on context compaction, artifact-memory reuse, and durable handoff quality for repo-scale review and development work.
...
record_scenario "RT05" "Compaction Pressure Architecture Memo" "$RT05_RAW" "${ARTIFACT_DIR}/rt05-compaction-memo.md" "$(extract_session_id "$RT05_RAW")"
...
Keep unresolved questions separate from completed tasks.
Then call finish with a one-line summary."
...
./bin/go-cli-agent continue "$RT13_SESSION_ID" \
	...
	--model "$MODEL" \
	--json \
	--message "Prioritize desktop onboarding. Ignore reports/ while gathering evidence. Write reports/continue-brief.md with sections: chosen priority, supporting evidence, next steps, unresolved questions. Then call finish." \
...
Inspect only README.md, AGENTS.md, spec/04-tools-and-skills.md, spec/13-live-input-and-steering.md, internal/runtime/prompt.go, and internal/tools/registry.go.
...
Write ${ABS_ARTIFACT_DIR}/rt14-steer-audit.md with sections: confirmed behavior, finding records, remaining risks, smallest next fixes.
Then call finish."
...
if kill -0 "$RT14_PID" 2>/dev/null; then
	...
	if (( RT14_ARTIFACT_WRITTEN_BEFORE_SECOND_STEER == 0 && RT14_FINISH_OBSERVED_BEFORE_SECOND_STEER == 0 )); then
...
Inspect only README.md, AGENTS.md, spec/00-product.md, spec/01-runtime-architecture.md, spec/03-provider-contracts.md, spec/11-spec-audit-and-traceability.md, spec/12-task-system.md, spec/13-live-input-and-steering.md, cmd/go-cli-agent/main.go, internal/app, internal/runtime, internal/provider, internal/session, and internal/tools.
Ignore validation/runs, validation/sessions, reports, bin, tmp, and generated artifacts.
Write ${ABS_ARTIFACT_DIR}/rt15-traceability-map.md with sections: core promise to owning files, strongest existing tests, unresolved questions, smallest next fixes.
Then call finish with a one-line summary."
...
Write ${ABS_ARTIFACT_DIR}/rt16-codex-steer-audit.md with sections: confirmed codex patterns, findings or cautions, ideas worth borrowing.
Then call finish with a one-line summary."
...
Write ${ABS_ARTIFACT_DIR}/rt17-codex-proxy-audit.md with sections: confirmed contracts, findings or mismatch risks, hardening ideas worth importing.
Then call finish with a one-line summary."
...
Write ${ABS_ARTIFACT_DIR}/rt18-opencode-task-review.md with sections: confirmed strengths, tradeoffs or findings, ideas go-cli-agent should adopt.
Then call finish with a one-line summary."
...
Write ${ABS_ARTIFACT_DIR}/rt19-opencode-responses-audit.md with sections: confirmed behavior, findings or mismatch risks, useful implementation ideas.
Then call finish with a one-line summary."
...
Also inspect only these source references: go-cli-agent/README.md, go-cli-agent/AGENTS.md, go-cli-agent/spec/00-product.md, go-cli-agent/spec/01-runtime-architecture.md, go-cli-agent/spec/11-spec-audit-and-traceability.md, go-cli-agent/spec/12-task-system.md, go-cli-agent/spec/13-live-input-and-steering.md, ${TOP_LEVEL_MARKDOWN_LIST}, codex/README.md, and opencode/README.md.
Use this live evidence to judge whether go-cli-agent is ready to surpass codex and opencode on large-project development and review.
Write ${ABS_ARTIFACT_DIR}/rt20-large-project-readiness.md with sections: live strengths, scorecard, remaining gaps, next architectural moves.
...
record_scenario "RT20" "Large Project Readiness Scorecard" "$RT20_RAW" "${ARTIFACT_DIR}/rt20-large-project-readiness.md" "$(extract_session_id "$RT20_RAW")"
